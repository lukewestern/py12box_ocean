...
from py12box_ocean import core, util, get_data, model
...
def get_ocean_parameters(species, ocean_file_str="species_ocean_info.csv"):
    """Get piston velocity and ocean hydrolysis half-life for ocean transport

    Args:
        species (str): Species name
        ocean_file_str (str, optional): File containing ocean info. Defaults to "species_ocean_info.csv".

    Returns:
        flaot, ndarray: piston velocity and ocean hydrolysis half-life for species
    """  
    ocean_file_str="species_ocean_info.csv"
    df_ocean = pd.read_csv(get_data("inputs") / ocean_file_str,
                        index_col="Species", comment="#", skipinitialspace=True)
    piston_velocity = df_ocean["piston_velocity"][species]/100/3600 # Put into m/s 
    half_life_mixed = df_ocean["half_life_mixed"][species]
    half_life_deep = df_ocean["half_life_deep"][species]
    return piston_velocity, np.array([half_life_mixed, half_life_deep])   
