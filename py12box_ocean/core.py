...
#@njit()
def ocean_transport_matrix(piston_velocity):
    """4 box model of ocean from Wang et al. https://doi.org/10.1073/pnas.2021528118"""
    day_to_sec = 24.0 * 3600.0
    year_to_sec = 365.25 * day_to_sec
    Ao = 0.85*361*(10**6)*(10**6)/2.  # ocean surface area of each box (unit in m^2), 85% covered in ice
    Vatm = 5.1170001e+18 * 1000 * 0.125 / 1293 # Approx volume of lower boxes 
    thermo_depth = 150
    kw = piston_velocity # Piston velocity in m/s
    Vocn = thermo_depth*Ao*0.5 # Volume of ocean in mixed layer box

    Tm = 1/(50*year_to_sec) # Inter-hemisphere, mixed layer
    # Tmd = 1/(3*year_to_sec) # Mixed layer -> deep ocean
    # Tdm = 1/(5*year_to_sec) # deep ocean -> Mixed layer
    Tmd = 1/(20*year_to_sec) # Mixed layer -> deep ocean
    Tdm = 1/(150*year_to_sec) # deep ocean -> Mixed layer
    Td = 1/(100*year_to_sec) # deep ocean inter hemisphere
    ...
    A[4,0] = kw*Ao/Vatm
    A[4,1] = kw*Ao/Vatm
    A[4,4] = -2*kw*Ao/Vocn + Tm - Tmd  
    A[4,5] = -Tm
    A[4,6] = Tdm
    A[5,2] = kw*Ao/Vatm
    A[5,3] = kw*Ao/Vatm
    A[5,4] = -Tm
    A[5,5] = -2*kw*Ao/Vocn + Tm - Tmd 
    A[5,7] = Tdm
    A[6,6] = -Tdm + Td
    A[6,4] = Tmd 
    A[6,7] = -Td
...
def model(ic, q, mol_mass, lifetime,
    ...
    piston_velocity : float
        Piston velocity (air-sea exchange velocity) of the gas (m/s)
        Default = 1
    half_life : ndarray
        Array of hydrolysis half-life of compound in years. 
        First terms is mixed layer hydrolysis, second is deep-ocean hydrolysis.
        Default = [1e6,1e6]
    ...
    n_box = len(mass)
    n_ocean_box = 4
    A = ocean_transport_matrix(piston_velocity)
    ...
    c = np.concatenate((c, np.zeros(4)))            # For now, hardwire zero ics for ocean
    ...
    c_month = np.zeros((n_months, n_box+n_ocean_box))
    cnt_month = np.zeros((n_months, n_box+n_ocean_box))
    cnt_global_month = np.zeros(n_months)

    loss_oh = np.zeros((n_months, n_box))
    ocean_flux = np.zeros(n_months)
    ...
    for ti in range(start_ti, end_ti):
        ...
        if not mi_ti_cnt:
            ...
            c_mi = np.zeros(n_box+n_ocean_box)
            cnt_mi = np.zeros(n_box+n_ocean_box)
        ...
        # Step forward solver
        c[:12] = model_solver_rk4(c[:12] / mass, F_mi, dt) * mass

        c_ocean = model_solver_rk4(np.concatenate((c[:4],c[12:])) , A, dt) 
        ocean_flux_mi += (c[:4] - c_ocean[:4]).sum()
        c[:4] = c_ocean[:4]
        # Ocean Hydrolysis as a half-life
        c_ocean[4:6] = c_ocean[4:6] * 0.5**(dt/(half_life[0]*year_to_sec))
        c_ocean[6:] = c_ocean[6:]   * 0.5**(dt/(half_life[1]*year_to_sec)) 
        c[12:] = c_ocean[4:]

        # Loss (g)
        loss_oh_ti = c[:12] * loss_oh_factor_mi
        loss_cl_ti = c[:12] * loss_cl_factor_mi
        loss_other_ti = c[:12] * loss_other_factor_mi
        ...
        # Update burden
        c[:12] += q_mi - loss_oh_ti - loss_cl_ti - loss_other_ti
        ...
        # lifetimes in s
        lifetime_oh_mi += c[:12] / loss_oh_ti * dt_scaled
        lifetime_cl_mi += c[:12] / loss_cl_ti * dt_scaled
        lifetime_other_mi += c[:12] / loss_other_ti * dt_scaled
        lifetime_total_mi += c[:12] / (loss_oh_ti + loss_cl_ti + loss_other_ti) * dt_scaled

        global_lifetime_oh_mi += c[:12].sum() / max([loss_oh_ti.sum(), 1.0e-30]) * dt_scaled
        global_lifetime_cl_mi += c[:12].sum() / max([loss_cl_ti.sum(), 1.0e-30]) * dt_scaled
        global_lifetime_strat_mi += c[:12].sum() / max([loss_other_ti[8:].sum(), 1.0e-30]) * dt_scaled
        global_lifetime_othertrop_mi += c[:12].sum() / max([loss_other_ti[0:8].sum(), 1.0e-30]) * dt_scaled
        global_lifetime_othertroplower_mi += c[:12].sum() / max([loss_other_ti[0:4].sum(), 1.0e-30]) * dt_scaled
        global_lifetime_total_mi += c[:12].sum() / max([(loss_oh_ti + loss_cl_ti + loss_other_ti).sum(), 1.0e-30]) * dt_scaled
    ...
    mole_fraction_out = np.divide(burden_out[:,:12] / mol_mass * mol_m_air * 1.0e12, mass)  # ppt

    mole_fraction_restart = np.divide(c_restart[:,:12] / mol_mass * mol_m_air * 1.0e12, mass)  # ppt

    # Emissions in g
    q_out = q_g * mi_ti  # g

    # Losses in g
    losses = {"oh": loss_oh, "cl": loss_cl, "other": loss_other, "ocean_flux":ocean_flux}
